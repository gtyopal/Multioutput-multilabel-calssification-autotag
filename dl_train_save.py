...
mode_name = "cnn"
mode_list = ["cnn","rnn","rnn_mean","rnn_att","rnn_cnn","cnn_merge","3layer_cnn"]
...
with tf.Graph().as_default():
    ...
    with tf.variable_scope("embedding_layer"):
        word_embedding = tf.get_variable('embedding', shape=embedding_weight.shape,
                                         initializer=tf.constant_initializer(value=embedding_weight))
    ...
    with tf.variable_scope("inter_layer"):
        if "rnn" in mode:
            fw_cell = tf.nn.rnn_cell.DropoutWrapper(tf.nn.rnn_cell.BasicLSTMCell(rnn_size, forget_bias=2.0),
                                                    output_keep_prob=dropout_prob)
            bw_cell = tf.nn.rnn_cell.DropoutWrapper(tf.nn.rnn_cell.BasicLSTMCell(rnn_size, forget_bias=2.0),
                                                    output_keep_prob=dropout_prob)
            ...
            if mode == "rnn_mean":
                ...
                sign_mask = tf.sequence_mask(lengths=tf.to_int32(input_x_len), maxlen=tf.to_int32(MAXLEN),
                                             dtype=tf.float32)
                sign_mask = tf.expand_dims(sign_mask, -1)
                birnn_output_mask = birnn_output * sign_mask
                birnn_output_sum = tf.reduce_sum(birnn_output_mask, axis=1)
                birnn_encoder = birnn_output_sum / tf.cast(tf.expand_dims(input_x_len, -1), tf.float32)
                hidden_ouput = birnn_encoder
                # self.bilstm_output.set_shape([None, self.hidden_neural_size*2])

            if mode == "rnn_att":
                birnn_output = tf.concat((fw_output, bw_output), axis=-1, name="concat_birnn_output")
                birnn_output_tanh = tf.layers.dense(birnn_output, rnn_size * 2, activation=tf.tanh)
                birnn_one = tf.layers.dense(birnn_output_tanh, 1)
                birnn_one_exp = tf.exp(birnn_one)
                sign_mask = tf.sequence_mask(lengths=tf.to_int32(input_x_len), maxlen=tf.to_int32(MAXLEN),
                                             dtype=tf.float32)
                sign_mask = tf.expand_dims(sign_mask, -1)
                birnn_one_exp = birnn_one_exp * sign_mask
                birnn_one_sum = tf.reduce_sum(birnn_one_exp, 1, keep_dims=True)
                birnn_res = birnn_one_exp / (birnn_one_sum + 1e-8)
                birnn_output_good = tf.reduce_sum(birnn_res * birnn_output, 1)
                hidden_ouput = birnn_output_good
            if mode == "rnn_cnn":
                birnn_output = tf.concat((fw_output, bw_output), axis=-1, name="concat_birnn_output")
                ...
                for i, filter_size in enumerate(filter_sizes):
                    with tf.name_scope("conv-maxpool-%s" % filter_size):
                        ...
                        input_conv = tf.nn.conv2d(cnn_input, cnn_weight1, strides=[1, 1, 1, 1], padding="VALID",
                                                  name="cnn_conv1")
            ...
            for i, filter_size in enumerate(filter_sizes):
                with tf.name_scope("conv-maxpool-%s" % filter_size):
                    filter_shape1 = [filter_size, embedded_size, 1, num_filters[i]]
                    cnn_weight1 = tf.Variable(tf.truncated_normal(filter_shape1, stddev=0.1), name="cnn_weith1")
                    cnn_biase1 = tf.Variable(tf.constant(0.1, shape=[num_filters[i]]), name="cnn_biase1")
                    input_conv1 = tf.nn.conv2d(cnn_input, cnn_weight1, strides=[1, 1, 1, 1], padding="VALID",
                                               name="cnn_conv1")
                    input_func1 = tf.nn.relu(tf.add(input_conv1, cnn_biase1), name="cnn_func1")
                    conv_out1 = tf.transpose(input_func1, [0, 1, 3, 2], name='conv_trans1')

                    filter_shape2 = [filter_size, num_filters[i], 1, num_filters[i]]
                    cnn_weight2 = tf.Variable(tf.truncated_normal(filter_shape2, stddev=0.1), name="cnn_weith2")
                    cnn_biase2 = tf.Variable(tf.constant(0.1, shape=[num_filters[i]]), name="cnn_biase2")
                    input_conv2 = tf.nn.conv2d(conv_out1, cnn_weight2, strides=[1, 1, 1, 1], padding="VALID",
                                               name="cnn_conv2")
                    input_func2 = tf.nn.relu(tf.add(input_conv2, cnn_biase2), name="cnn_func2")
                    conv_out2 = tf.transpose(input_func2, [0, 1, 3, 2], name='conv_trans2')

                    filter_shape3 = [filter_size, num_filters[i], 1, num_filters[i]]
                    cnn_weight3 = tf.Variable(tf.truncated_normal(filter_shape3, stddev=0.1), name="cnn_weith3")
                    cnn_biase3 = tf.Variable(tf.constant(0.1, shape=[num_filters[i]]), name="cnn_biase3")
                    input_conv3 = tf.nn.conv2d(conv_out2, cnn_weight3, strides=[1, 1, 1, 1], padding="VALID",
                                               name="cnn_conv3")
                    input_func3 = tf.nn.relu(tf.add(input_conv3, cnn_biase3), name="cnn_func3")

                    cnn_pooled = tf.reduce_max(tf.squeeze(input_func3, [2]), 1)
                    cnn_output.append(cnn_pooled)

            filters_total = sum(num_filters)
            cnn_full_dim = filters_total / 2
            cnn_output_flat = tf.reshape(tf.concat(values=cnn_output, axis=1), [-1, filters_total])
            cnn_full_w = tf.get_variable("cnn_outW", [filters_total, cnn_full_dim], dtype=tf.float32)
            cnn_full_b = tf.get_variable("cnn_outB", [cnn_full_dim], dtype=tf.float32)
            cnn_trans = tf.nn.xw_plus_b(cnn_output_flat, cnn_full_w, cnn_full_b)
            batch_mean, batch_var = tf.nn.moments(cnn_trans, [0])
            scale2 = tf.Variable(tf.ones([cnn_full_dim]))
            beta2 = tf.Variable(tf.zeros([cnn_full_dim]))
            cnn_batchout = tf.nn.batch_normalization(cnn_trans, batch_mean, batch_var, beta2, scale2,
                                                     variance_epsilon=1e-3)
            cnn_transform_output = tf.nn.relu(cnn_batchout)
            cnn_output_drop = tf.nn.dropout(cnn_transform_output, dropout_prob)
            hidden_ouput = cnn_output_drop

        elif mode == "cnn_merge":
            cnn_input1 = tf.expand_dims(input_embed, -1)
            num_filters = [128, 128, 128, 128, 128]
            filter_sizes = [1, 2, 3, 4, 5]
            filters_total = sum(num_filters)
            cnn_output1 = []
            for i, filter_size in enumerate(filter_sizes):
                with tf.name_scope("1conv-maxpool-%s" % filter_size):
                    filter_shape = [filter_size, embedded_size, 1, num_filters[i]]
                    cnn_weight1 = tf.Variable(tf.truncated_normal(filter_shape, stddev=0.1), name="cnn_weith1")
                    cnn_bias1 = tf.Variable(tf.constant(0.1, shape=[num_filters[i]]), name="cnn_bias1")

                    input_conv = tf.nn.conv2d(cnn_input1, cnn_weight1, strides=[1, 1, 1, 1], padding="VALID",
                                              name="cnn_conv1")
                    input_func = tf.nn.relu(tf.add(input_conv, cnn_bias1), name="cnn_func1")
                    input_pooled = tf.reduce_max(tf.squeeze(input_func, [2]), 1)
                    cnn_output1.append(input_pooled)
            cnn_output_flat1 = tf.reshape(tf.concat(values=cnn_output1, axis=1), [-1, filters_total])
            cnn_output_drop1 = tf.nn.dropout(cnn_output_flat1, dropout_prob)

            input_embed_transpose = tf.transpose(input_embed, [0, 2, 1])
            cnn_input2 = tf.expand_dims(input_embed_transpose, -1)
            cnn_output2 = []
            cnn_size = MAXLEN
            for i, filter_size in enumerate(filter_sizes):
                with tf.name_scope("2conv-maxpool-%s" % filter_size):
                    filter_shape = [filter_size, cnn_size, 1, num_filters[i]]
                    cnn_weight1 = tf.Variable(tf.truncated_normal(filter_shape, stddev=0.1), name="cnn_weith1")
                    cnn_bias1 = tf.Variable(tf.constant(0.1, shape=[num_filters[i]]), name="cnn_bias1")

                    input_conv = tf.nn.conv2d(cnn_input2, cnn_weight1, strides=[1, 1, 1, 1], padding="VALID",
                                              name="cnn_conv1")
                    input_func = tf.nn.relu(tf.add(input_conv, cnn_bias1), name="cnn_func1")
                    input_pooled = tf.reduce_max(tf.squeeze(input_func, [2]), 1)
                    cnn_output2.append(input_pooled)
            filters_total = sum(num_filters)
            cnn_output_flat2 = tf.reshape(tf.concat(values=cnn_output2, axis=1), [-1, filters_total])
            cnn_output_drop2 = tf.nn.dropout(cnn_output_flat2, dropout_prob)

            cnn_output_drop = tf.concat((cnn_output_drop1, cnn_output_drop2), axis=-1)
            hidden_ouput = cnn_output_drop
    ...
    with tf.variable_scope("output_layer"):
        ...
        softmax_weight = tf.get_variable("weight", [dense_size * 2, label_size],
    ...
    def train():
        ...
        for epoch in range(train_epoch):
            training_batches = generate_batch(train_message, train_message_len, train_label, batch_size,
                                              random_flag=True)
            ...
            for batch_x, batch_x_len, batch_y in training_batches:
                ...
                print("Train, step {}, loss {:g}, acc {:g}, step-time {:g}, examples/sec {:g}".format(global_step_tmp,
                                                                                                      train_loss,
                                                                                                      train_accuracy,
                                                                                                      step_time,
                                                                                                      sample_psec))
            ...
            print("Test, loss {:g}, acc {:g}, step-time {:g}, examples/sec {:g}".format(test_loss, test_accuracy,
                                                                                        step_time, sample_psec))
